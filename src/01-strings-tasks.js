...
function getRectangleString(width, height) {
  const borderCenter = '─'.repeat(width - 2);
  const centralCenter = ' '.repeat(width - 2);
  const topBorder = `┌${borderCenter}┐\n`;
  const centralBorder = `│${centralCenter}│\n`;
  const centrapPart = centralBorder.repeat(height - 2);
  const bottomBorder = `└${borderCenter}┘\n`;
  return topBorder + centrapPart + bottomBorder;
}
